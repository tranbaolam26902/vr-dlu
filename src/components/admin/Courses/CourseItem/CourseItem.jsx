...
export default function CourseItem({ course }) {
    ...
    const handleToggleDelete = async () => {
        ...
        if (toggleResult.isSuccess) {
            if (course.isDeleted) toast.success('Khôi phục thành công.');
            else toast.success('Chuyển vào thùng rác thành công.');
            dispatch(triggerUpdateCourses());
        } else {
            if (course.isDeleted) toast.error('Khôi phục thất bại.');
            else toast.error('Chuyển vào thùng rác thất bại.');
        }
    };
    const handleDelete = async () => {
        if (!confirm('Xác nhận xoá chủ đề?')) return;
