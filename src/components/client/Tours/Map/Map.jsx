    ...
    return (
        <Modal show={tourSlice.showMap} handleClose={handleCloseMap}>
            <section className='relative max-w-[80vw] max-h-[80vh]'>
                <img
                    src={`${import.meta.env.VITE_API_ENDPOINT}/${map.path}`}
                    alt={map.bb}
                    className='max-h-[80vh] border border-white rounded-lg'
                />
                <div className='absolute inset-5'>
                    {map.pins.map((pin) => (
                        ...
                        >
                            ...
                            <span className='absolute z-20 left-1/2 bottom-full -translate-x-1/2 px-4 w-max font-semibold text-sm text-white bg-gray-600 rounded'>
                                {pin.title}
                            </span>
                            {pin.thumbnailPath && (
                                <div className='hidden group-hover:block absolute -top-full left-1/2 -translate-x-1/2 -translate-y-2/3 w-28'>
                                    <img
