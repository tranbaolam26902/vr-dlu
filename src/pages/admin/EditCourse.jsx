...
export default function EditCourse() {
    ...
    const setPageTitle = () => {
        if (loaderData) document.title = getSubPageTitle('Chỉnh sửa chủ đề');
        else document.title = getSubPageTitle('Thêm chủ đề');
    };
    ...
    return (
        <section className='min-h-[calc(100vh-3.75rem)] dark:bg-black dark:text-white'>
            <Container className='flex flex-col gap-y-4 py-4'>
                <h1 className='font-semibold text-2xl'>{slug ? 'Chỉnh sửa chủ đề' : 'Thêm chủ đề mới'}</h1>
