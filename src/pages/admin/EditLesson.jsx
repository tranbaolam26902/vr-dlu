...
export default function EditLesson() {
    ...
    const setPageTitle = () => {
        if (loaderData) document.title = getSubPageTitle('Chỉnh sửa điểm thông tin');
        else document.title = getSubPageTitle('Thêm điểm thông tin');
    };
    const getTours = () => {
    ...
    return (
        <section className='min-h-[calc(100vh-3.75rem)] dark:bg-black dark:text-white'>
            <Container className='flex flex-col gap-y-4 py-4'>
                <h1 className='font-semibold text-2xl'>{id ? 'Chỉnh sửa điểm thông tin' : 'Thêm điểm thông tin mới'}</h1>
                <div className='grid grid-cols-1 lg:grid-cols-2 gap-x-8'>
                    <form className='flex flex-col gap-y-4' onSubmit={handleSaveLesson}>
                        ...
                        <Input
                            ...
                            id='title'
                            label='Tên'
                            placeholder='Nhập tên điểm thông tin...'
                            required
                            value={title}
