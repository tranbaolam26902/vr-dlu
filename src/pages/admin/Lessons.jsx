...
export default function Lessons() {
    ...
    const getAllCourses = async () => {
        ...
        if (coursesResult.isSuccess) {
            ...
            if (coursesResult.result.length > 0 && !urlCourseId && !location.state.courseId) setCourseId(coursesResult.result[0].id);
        } else setCourses([]);
    };
    ...
    return (
        <section className='min-h-[calc(100vh-3.75rem)] dark:bg-black dark:text-white'>
            <Container className='flex flex-col gap-4 py-4'>
                <h1 className='font-semibold text-2xl'>Danh sách điểm thông tin</h1>
